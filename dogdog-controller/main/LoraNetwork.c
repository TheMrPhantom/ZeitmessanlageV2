...
void log_dogdog_packet(DogDogPacket *packet)
{
    ...
    ESP_LOGI(pcTaskGetName(NULL), "DogDogPacket: magic=0x%04X, protocol_version=%d, station_id=%d, packet_id=%d, type=%s, length=%d",
             (unsigned int)packet->magic, packet->protocol_version, packet->station_id, packet->packet_id, packet_type_str, packet->length);
...
{
    gpio_reset_pin(LORA_GPIO_DIO0);
    gpio_set_direction(LORA_GPIO_DIO0, GPIO_MODE_INPUT);
    gpio_set_intr_type(LORA_GPIO_DIO0, GPIO_INTR_POSEDGE);
    gpio_isr_handler_add(LORA_GPIO_DIO0, lora_module_rx_isr, (void *)LORA_GPIO_DIO0);
