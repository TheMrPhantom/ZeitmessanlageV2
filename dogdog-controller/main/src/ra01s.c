...
void LoRaInit(void)
{
	ESP_LOGI(TAG, "CONFIG_MISO_GPIO=%d", LORA_GPIO_MISO);
	ESP_LOGI(TAG, "CONFIG_MOSI_GPIO=%d", LORA_GPIO_MOSI);
	ESP_LOGI(TAG, "CONFIG_SCLK_GPIO=%d", LORA_GPIO_SCLK);
	ESP_LOGI(TAG, "CONFIG_NSS_GPIO=%d", LORA_GPIO_NSS);
	ESP_LOGI(TAG, "CONFIG_RST_GPIO=%d", LORA_GPIO_RST);
	ESP_LOGI(TAG, "CONFIG_BUSY_GPIO=%d", LORA_GPIO_BUSY);
	ESP_LOGI(TAG, "CONFIG_TXEN_GPIO=%d", LORA_GPIO_TXEN);
	ESP_LOGI(TAG, "CONFIG_RXEN_GPIO=%d", LORA_GPIO_RXEN);

	SX126x_SPI_SELECT = LORA_GPIO_NSS;
	SX126x_RESET = LORA_GPIO_RST;
	SX126x_BUSY = LORA_GPIO_BUSY;
	SX126x_TXEN = LORA_GPIO_TXEN;
	SX126x_RXEN = LORA_GPIO_RXEN;
	...
	spi_bus_config_t spi_bus_config = {
		.sclk_io_num = LORA_GPIO_SCLK,
		.mosi_io_num = LORA_GPIO_MOSI,
		.miso_io_num = LORA_GPIO_MISO,
		.quadwp_io_num = -1,
		.quadhd_io_num = -1};

	esp_err_t ret;
	ret = spi_bus_initialize(LORA_SPI_HOST, &spi_bus_config, SPI_DMA_CH_AUTO);
	...
	spi_device_interface_config_t devcfg = {
		...
		.mode = 0,
		.spics_io_num = LORA_GPIO_NSS,
		.queue_size = 7,
		.flags = 0,
		.pre_cb = NULL};
	// spi_device_handle_t handle;
	ret = spi_bus_add_device(LORA_SPI_HOST, &devcfg, &SpiHandle);
