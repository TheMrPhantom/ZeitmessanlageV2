...
timeval_t last_start_trigger;
timeval_t last_stop_trigger;
...
void Sensor_Interrupt_Task(void *params)
{
    ...
    gettimeofday(&last_start_trigger, NULL);
    gettimeofday(&last_stop_trigger, NULL);
    init_Pins();
            ...
            if (gpio_get_level(pinNumber) == 1)
            {
                ...
                if (lastTriggerTime < (int)pdTICKS_TO_MS(xTaskGetTickCount()) - sensorCooldown)
                {
                    if (!fault)
                    {
                        ...
                        {
                            last_start_trigger = current_time;
                        }
                        else if (pinNumber == TRIGGER_PIN_2)
                        {
                            last_stop_trigger = current_time;
                        }
