...
void LoRaInit(void)
{
	...
	ESP_LOGI(TAG, "CONFIG_NSS_GPIO=%d", CONFIG_LORA_GPIO_NSS);
	ESP_LOGI(TAG, "CONFIG_RST_GPIO=%d", CONFIG_LORA_GPIO_RST);
	ESP_LOGI(TAG, "CONFIG_BUSY_GPIO=%d", CONFIG_LORA_GPIO_BUSY);
	ESP_LOGI(TAG, "CONFIG_TXEN_GPIO=%d", CONFIG_LORA_GPIO_TXEN);
	ESP_LOGI(TAG, "CONFIG_RXEN_GPIO=%d", CONFIG_LORA_GPIO_RXEN);

	SX126x_SPI_SELECT = CONFIG_LORA_GPIO_NSS;
	SX126x_RESET = CONFIG_LORA_GPIO_RST;
	SX126x_BUSY = CONFIG_LORA_GPIO_BUSY;
	SX126x_TXEN = CONFIG_LORA_GPIO_TXEN;
	SX126x_RXEN = CONFIG_LORA_GPIO_RXEN;
	...
	spi_bus_config_t spi_bus_config = {
		.sclk_io_num = CONFIG_LORA_GPIO_SCLK,
		.mosi_io_num = CONFIG_LORA_GPIO_MOSI,
		.miso_io_num = CONFIG_LORA_GPIO_MISO,
		.quadwp_io_num = -1,
		.quadhd_io_num = -1};
	...
	spi_device_interface_config_t devcfg = {
		...
		.mode = 0,
		.spics_io_num = CONFIG_LORA_GPIO_NSS,
		.queue_size = 7,
		.flags = 0,
		.pre_cb = NULL};
